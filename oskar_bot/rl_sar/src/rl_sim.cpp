...
void RL_Sim::CmdvelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
    ...
    this->scaled_cmd_vel = ScaleVelocity(*msg);

    if (abs(msg->linear.x) > 0.0001 || abs(msg->angular.z) > 0.001) {
        RCLCPP_INFO(this->get_logger(), "Original: x=%.6f, z=%.6f | Scaled: x=%.2f, z=%.2f",
                    msg->linear.x, msg->angular.z,
                    this->scaled_cmd_vel.linear.x, this->scaled_cmd_vel.angular.z);
    }
}

//z값(회전)만 스케일업
geometry_msgs::msg::Twist RL_Sim::ScaleVelocity(const geometry_msgs::msg::Twist& original_cmd)
{
    geometry_msgs::msg::Twist scaled_cmd = original_cmd;

    scaled_cmd.linear.x = original_cmd.linear.x;
    scaled_cmd.linear.y = original_cmd.linear.y;
    scaled_cmd.linear.z = original_cmd.linear.z;

    const double MIN_ANGULAR_VEL = 1.5;
    const double MAX_ANGULAR_VEL = 2.0;

    if (abs(original_cmd.angular.z) > 0.001) {
        if (original_cmd.angular.z > 0) {
            // 양수: 최소값보다 작으면 스케일업
            scaled_cmd.angular.z = std::max(original_cmd.angular.z * 10.0, MIN_ANGULAR_VEL);
        } else {
            // 음수: 최소값보다 작으면 스케일업
            scaled_cmd.angular.z = std::min(original_cmd.angular.z * 10.0, -MIN_ANGULAR_VEL);
        }
        // 최대값 제한
        scaled_cmd.angular.z = std::max(-MAX_ANGULAR_VEL, std::min(MAX_ANGULAR_VEL, scaled_cmd.angular.z));
    } else {
        scaled_cmd.angular.z = 0.0;
    }
    ...
    if (this->rl_init_done && simulation_running)
    {
        ...
        if (this->control.navigation_mode)
        {
            ...
            this->obs.commands = torch::tensor({
                {this->scaled_cmd_vel.linear.x, 
                 this->scaled_cmd_vel.linear.y, 
                 this->scaled_cmd_vel.angular.z}
            });
        }
        else
        {
